...
import { PasswordlessResponseWithDriverDetails } from '@cd/core-lib';
import {TextContent} from '@cd/core-lib';
import { driverActions } from '@cd/core-lib';
...
const PasscodeView = ({ route }) => {
	...
	const {
		...
		mode: 'onSubmit',
		resolver: yupResolver(passcodeValidationSchema),
	});

	const onSubmit = async (data: any) => {
		try {
			let response = await axios.post<PasswordlessResponseWithDriverDetails>(
				urlBuilder.main.submitOTP(),
				{
	...
	return (
		<View className="bg-primary h-full">
			...
			<Center className="w-[300px] m-auto">
				...
				<View
					className={twMerge(RNstyles.textfield.inputContainer)}
				...
				>
					<H5 color="light">{TextContent.ui.CONTINUE}</H5>
				</Button>
			</Center>
		</View>
	);
...
const passcodeValidationSchema = yup.object().shape({
	passcode: yup.string().required('Invalid passcode.'),
});
