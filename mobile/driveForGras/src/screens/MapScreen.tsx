...
import {
	driverActions,
	selectDriverState,
} from '@cd/core-lib/src/reduxDir/features/driver.reducer';
import {
	selectSocketState,
	socketActions,
} from '@cd/core-lib/src/reduxDir/features/socket.reducer';
...
const MapScreen = () => {
	...
	const {
		...
		{ connectionOpenInit, isConnected, errorMessage, incomingOrder } =
	...
	const isGoingOnline = updateStatus || (!isConnected && connectionOpenInit),
		showOnlineStatus = isGoingOnline
			? 'Going Online...'
			: isConnected
	...
	return (
		<>
			<>
				...
				<Text>
					{' '}
					connectionOpenInit: {connectionOpenInit ? 'true' : 'false'}
				</Text>
				<Text> error: {errorMessage || 'null'}</Text>
			</>

			<View className="grow">
				<View className="absolute top-0 right-0 z-10 p-1">
					<Paragraph className="font-bold text-inverse">Gras</Paragraph>
				</View>
				...
				<BannerButton onPress={toggleStatus} disabled={false}>
					{showOnlineStatus}
				</BannerButton>
			</View>
