...
type GenerateInitialValues<T> = T extends Record<string, infer U>
  ? {
...
type RecursiveGenerateInitialValues<T> = T extends Record<string, any> ? GenerateInitialValues<T> : never;
...
function generateInitialValues<T>(): GenerateInitialValues<T> {
  ...
  for (const key in initialValues) {
    if (initialValues.hasOwnProperty(key)) {
      initialValues[key] = typeof initialValues[key] === 'number'
        ? 0
        : typeof initialValues[key] === 'boolean'
        ? false
        : Array.isArray(initialValues[key])
        ? (initialValues[key] as any).map(() => generateInitialValues<typeof initialValues[key][0]>())
...
function composeSnippet<T extends Record<string, any>>(): Record<string, SNIPPETBODY> {
    return{
    SNIPPET_NAME: {
...
//     type InputType = { title: string; name: string; price: number; isExpired?: boolean };
    ...
    function generateYupSchema<T extends Record<string, any>>(): yup.ObjectSchema<T> {
