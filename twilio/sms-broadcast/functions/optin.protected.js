...
const helpMessage = (name) =>
	name ? `Text "join" to join ${name}.` : 'Text "join" to join.';
const unknownCommandMessage = () =>
	"Sorry, I didn't understand that. " + helpMessage();
const optInSuccessMessage = (name) =>
	name
		? `You're a member! You'll receive the hottest events and offers from ${name}.`
		: `You're a member! You'll receive the hottest events and offers in the city. 🌆`;
const optInFailMessage = () => "We couldn't subscribe you - try again later?";
const optOutSuccessMessage = () =>
	"You've been unsubscribed. We're sorry to see you go!";
const optOutFailMessage = () =>
...
class OptinCommand extends Command {
	async run(callback) {
		try {
			...
			const customer = await utils.retrieveCustomerData(
				this.context,
				this.fromNumber
			);
			...
			if (!customer) {
				...
				const response = await fetch(`${endpoint}`, {
					...
					body: JSON.stringify({
						...
						segment: this.accountData.segment,
					}),
					headers: {
				...
				const { id } = customer;
				await utils.storeCustomerData(this.context, this.fromNumber, {
					...customer,
					contactId: id,
				});
				await utils.storeCustomerSegment(
					this.context,
					this.fromNumber,
					this.accountData.segment
				);
			}

			await utils
				.retrieveCustomerSegmentList(this.context, this.fromNumber)
				.then(async (customerSegments) => {
					if (!customerSegments.includes(this.accountData.segment)) {
						if (!didUpdateCustomerCRM) {
							this.logger.info(
								`Adding user to segment: ${this.accountData.slug}`
							);
							this.logger.info(`POST to url: ${endpoint}`);
							fetch(`${endpoint}`, {
								method: 'POST',
						...
						return await utils
							...
							.then(() => {
								console.log(
									`Stored new segment ${this.accountData.segment} for ${this.fromNumber}`
								);
								return true; // Indicates a new segment was added
							});
					} else {
						console.log(
							`Segment ${this.accountData.segment} already exists for ${this.fromNumber}`
						);
			...
			this.logger.info(
				`The user was added to list: ${this.accountData.slug}. Contact id: ${customer.id}`
			);
...
class OptoutCommand extends Command {
	async run(callback) {
		try {
			...
			const customerData = await utils.retrieveCustomerData(
				this.context,
				this.fromNumber,
				this.accountData
			);
			...
			if (!response.ok) {
				const errorData = await response.json();
				throw new Error(
					errorData?.errors?.message[0] || 'Unsubscribe request failed'
				);
			}
			...
			await utils.removeCustomerSegment(
				this.context,
				this.fromNumber,
				this.accountData.segment
			);
...
exports.handler = async (context, event, callback) => {
	...
	await utils
		.retrieveAccountData(context, event.To)
		.then((account) => {
