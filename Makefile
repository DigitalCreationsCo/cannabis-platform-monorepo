...
docker-build-nocache-output:
	docker build -t grasadmin/$(BUILD_CONTEXT) -f Dockerfile.$(BUILD_TYPE) --build-arg=BUILD_TYPE=$(BUILD_TYPE) --build-arg=BUILD_CONTEXT=$(BUILD_CONTEXT) --build-arg=PORT=$(PORT) --no-cache . 2>&1 | tee build.log
