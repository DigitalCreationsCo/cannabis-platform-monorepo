...
export default function useCarousel(
  ...
  // show two shadow slides on each side
  const shadowSlides = 2 * slidesPresented;

  // get total number of slides to be presented
  ...
  useEffect(() => {
    ...
    console.log('state.desired', state.desired)
  ...
  const style: React.CSSProperties = {
    ...
    // width is the length of all slidges + 2 shadow slides
    width: `${totalWidth * (length + shadowSlides)}%`,

    left: `-${(state.active + 1) * (totalWidth + shadowSlides)}%`,
  };
  ...
  if (state.desired !== state.active) {
    ...
    if (state.offset !== 0) {
      ...
      console.log('offset transform', style.transform);
    } else {
      style.transition = elastic;
      console.log('elastic transform', style.transform);
    }
  }
