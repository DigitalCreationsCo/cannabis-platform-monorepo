...
const socketMiddleware = (store: MiddlewareAPI<_,AppState>) => {
	...
	const {socketMap} = store.getState().socket
	...
	return (next: any) => (action: AnyAction) => {
		...
		if (socketActions.openConnection.match(action)) {
			...
			await store.dispatch(socketActions.goOnline(true))

			dispatch_socket = getSocket('dispatch_socket');
			...
			dispatch_socket.on(
				...
				({ message, data }: SocketEventPayload<OrderWithDispatchDetails['order']>) => {
	...
	if (isActiveDelivery === true) {
		...
		for (socketKey in socketMap) {
		  ...
		  data: ${data}`,
				...
				_order_socket_connection.on(SocketEvent.navigate, ({ type }: { type: any }) => {
					switch (type) {
