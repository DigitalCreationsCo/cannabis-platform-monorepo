...
function Carousel({ 
    ...
    return (
        <div className="relative overflow-hidden">
            {title && 
            <H5 className='px-4 whitespace-normal'>
                {title}</H5>}

            <div className='px-4 overflow-hidden w-full h-full relative'>
                <Button size="sm"
                onClick={() => setActive(active > 1 ? active - 1 : active)}
                className="px-4 py-8 bg-inverse-soft hover:bg-accent-soft items-center relative justify-center active:bg-accent"
                // className={twMerge(styles.carouselButton, 'left-0')}
                >
                    <IconWrapper 
                    Icon={Icons.ChevronLeft}/></Button>

                <div 
                className={twMerge(styles.carousel)}
                ...
                className={twMerge(styles.carouselButton, 'right-0')}>
                    <IconWrapper 
                    Icon={Icons.ChevronRight}/></Button>
            </div>
        </div>
    );
}
