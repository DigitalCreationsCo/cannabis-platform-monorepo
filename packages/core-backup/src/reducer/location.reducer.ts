...
import { USStateAbbreviated } from '@gras/data-access';

const getAddressByCoordinates: AsyncThunk = createAsyncThunk(
...
const selectCurrentLocationState = (state: AppState) =>
	state.location.currentLocation.address.coordinates;

const selectSelectedLocationState = (state: AppState) => {
