...
function ProductItem({ product, className, handleConfirm }: ProductItemProps & PropsWithChildren) {
    ...
    return (
        <>
            <View className={twMerge('flex flex-row col-span-2 md:col-span-1 lg:col-span-2 xl:col-span-1 justify-stretch', className)}
                onResponderStart={toggleConfirm}
            >
                <Image source={product?.images?.[0]?.location as any} style={{ height: 100, maxHeight: 100, width: 100, maxWidth: 100 }} className='rounded border object-cover place-self-center' />
    ...
    function ProductItemModal() {
        ...
        >
            <TextInput
                ...
                // type="number"
                // value={quantity}
                onChange={(event) => setQuantity(event.target.valueOf as unknown as number)}
            />
        </ConfirmationModal>
        );
    }
