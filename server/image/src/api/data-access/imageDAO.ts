...
class ImageDAO {
	...
	async verifyIdentificationImage(
		...
		try {
			...
			const _text = await this.imageWorker.parseImageToText(
				image as Buffer
			);

			const { isLegalAge, scannedDOB } = new IdCardReader(
				_text
			).isLegalAge();
		...
		try {
			...
			await Promise.all(
				files.map(async (file): Promise<ImagePaths> => {
					...
					return {
						[file.fieldname]: this.getObjectStorageLocation(
							key,
							bucket
						),
					};
				...
				_uploadedFiles.forEach((file) =>
					Object.assign(uploadedFiles, file)
				);
			});
