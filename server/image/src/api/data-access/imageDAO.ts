...
class ImageDAO {
     ...
     * @returns 
     */
    async uploadToS3({ files, bucket }: { files: ImageFile[], bucket: string }): Promise<ImagePaths> {
        try {
            ...
            await Promise.all(files.map(async (file): Promise<ImagePaths> => {
                ...
                const
                    fileExtension = this.getExtensionFromMimeType(file.mimetype),
                ...
                return { [file.fieldname]: this.getObjectStorageLocation(key, bucket) };
            })
            ).then(
                _uploadedFiles => {
