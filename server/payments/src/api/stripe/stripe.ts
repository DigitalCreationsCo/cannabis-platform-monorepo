...
class StripeService {
	...
	async createCheckout(
		...
		try {
			let session = await this.stripe.checkout.sessions.create({
				mode: 'payment',
				success_url:
					process.env.NEXT_PUBLIC_SHOP_APP_CHECKOUT_SUCCESS_URL,
				...
				payment_intent_data: {
					application_fee_amount:
						calculatePlatformFeeForTransaction(123),
					transfer_data: {
	...
	async createDispensaryAccount(accountParams: Stripe.CustomerCreateParams) {
		try {
			if (!accountParams)
				throw new Error(
					'Dispensary Stripe Account Params are required!'
				);
	...
	async createDispensaryAccountLink(params: Stripe.AccountLinkCreateParams) {
		try {
			if (!params || !params.account)
				throw new Error(
					'Dispensary Stripe Account Link Params are required!'
				);
		...
		switch (event.type) {
			case 'checkout.session.completed':
				...
				const order = event.data.object
					.metadata as CheckoutSessionMetaData;
