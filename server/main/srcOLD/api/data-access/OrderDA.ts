...
export default class OrderDA {
  ...
  static async updateOnlineStatus(driverId, onlineStatus) {
    ...
    try {
      ...
      let statusResult = await driverSessions.updateOne(query, update);
      console.log(
  ...
  static async updateOrdersDriverPath(driverPathPacketList) {
    try {
      ...
      let updateResult = await orders.bulkWrite(updateBulkOp);
  ...
  static async getDriverSessionById(driverId) {
    try {
      ...
      const driver = await driverSessions.findOne(
        { driverId: ObjectId(driverId) },
        projection
      );
