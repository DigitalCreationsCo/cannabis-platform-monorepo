...
export default class OrganizationDAO {
  ...
  static async getPendingOrders(vendorId) {
    try {
      let query = { vendorId: ObjectId(vendorId) };
      return await pendingOrdersColl.find(query).toArray();
    } catch (e) {
      console.error(
  ...
  static async addOrder(order) {
    try {
      ...
      const result = await pendingOrdersColl.insertOne(orderDocument, {
        w: "majority",
      });
  ...
  static async deleteOrder(order) {
    try {
      let { orderId } = order;
      console.log(orderId);
      await pendingOrdersColl.deleteOne({ orderId });
      return { success: true };
    } catch (e) {
      console.error(`Error occurred while adding new order, ${e}.`);
      return { error: e };
  ...
  static async watchPendingOrders(vendorId) {
    ...
    return pendingOrdersColl.watch();
  }
}
