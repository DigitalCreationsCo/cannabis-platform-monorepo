...
describe('Crawler : Dutchie Cart', () => {
	it('regex returns the correct data from an item string', async () => {
		...
		const unit = string.match(regexFieldDict.unit)?.[2];
		expect(unit).toBe('g');
		...
		expect(string2.match(regexFieldDict.unit)?.[2]).toBe('g');
		...
		expect(string3.match(regexFieldDict.unit)?.[2]).toBe('g');
		...
		expect(string4.match(regexFieldDict.unit)?.[2]).toBe('mg');
		...
		expect(string5.match(regexFieldDict.unit)?.[2]).toBe(undefined);
		...
		expect(string6.match(regexFieldDict.unit)?.[2]).toBe('mg');
	});
});
