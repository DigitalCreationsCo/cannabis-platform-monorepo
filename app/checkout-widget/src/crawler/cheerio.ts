	...
	try {
		...
		function parseCartHtml() {
			...
			function getDomData(domain: string) {
				return (
					_domDataSelector[domain] || _domDataSelector['localhost']
				);
			}
			function getCartItemSelector() {
				return (
					_cartItemSelector[_domain] || _cartItemSelector['localhost']
				);
			}

			function getCartTotalSelector() {
				return (
					_cartTotalSelector[_domain] ||
					_cartTotalSelector['localhost']
				);
			}
						...
						images: [
							{
								...
								location: (
									$item
										.find(
											getDomData(_domain).images.location
										)
										.attr('src') || ''
