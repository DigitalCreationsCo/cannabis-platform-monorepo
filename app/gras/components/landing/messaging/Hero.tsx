...
import { FlexBox, H1, Paragraph, styles } from '@cd/ui-lib';
...
function Hero() {
	const [heading, largeHeading] = [
		'tracking-normal !leading-tight font-bold text-inverse-soft text-4xl lg:text-6xl xl:text-6xl drop-shadow-[0px_3px_1px_#444]',
	...
	return (
		<section className={twMerge('bg-gradient-to-b', 'from-secondary', 'to-primary'
		, 'py-8 pb-16 md:pt-16')}>
			<FlexBox className='flex-col md:flex-row justify-center items-center md:items-start gap-x-8 mx-auto px-16'>
...
<FlexBox className='flex-1 justify-center items-center max-w-2xl'>
			...
			<div className="sm:my-0 mx-auto grid py-6 place-self-center text-left gap-y-8 mt-0">
				...
				<FlexBox className="w-full py-8 gap-4 sm:flex-row lg:gap-8">
					<CTA cta={'Get Started'} href={'/auth/login'} />
				</FlexBox>
				</div>

			</FlexBox>
<Image src={messageImg} alt={`consumer-messaging`} className='rounded bg-inverse shadow-lg object-cover max-w-sm lg:max-w-md flex-1 md:mt-8' />
