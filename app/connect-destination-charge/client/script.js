...
fetch('/recent-accounts', {
	...
	.then((data) => {
		...
		if (!accounts.length) {
			document
				.querySelector('#no-accounts-section')
		...
		if (!enabledAccounts.length) {
			var expressAccounts = accounts.filter(
				(acct) => acct.type == 'express'
			);
			var hasCustom = !!accounts.filter((acct) => acct.type == 'custom');
			var hasStandard = !!accounts.filter(
				(acct) => acct.type == 'standard'
			);
...
expressDashboardForm.addEventListener(
	...
	(event) => {
		...
		params = {
			account_id: document.querySelector('#disabled-accounts-select')
				.value,
		};
