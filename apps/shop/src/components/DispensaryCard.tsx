...
function DispensaryCard({ data: dispensary, loading, className }: DispensaryCardProps) {
    ...
    return (
        ...
        <Card className={twMerge([styles.dispensarycard, className])}>
            ...
            >
                <H2 className='z-5 p-2 tracking-wide absolute top-0 left-0 text-inverse'>{dispensary?.name}</H2>
                ...
                <Paragraph className="text-inverse font-bold z-5 top-0 right-0 p-3 m-3 badge absolute">{checkDispensaryIsOpen(dispensary.schedule)}</Paragraph>
